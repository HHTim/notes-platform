...
- **HackMD 的網址優先走連接器**：先看工作階段裡有沒有 HackMD 的 MCP 工具（工具名含
  hackmd，例如 get-note）——有就用它拿內容，連結最後一段（如 `SJj8ApUw2`）就是筆記的
  短 ID。雲端工作階段的網路門禁擋 hackmd.io，抓網頁會失敗，但連接器走 claude.ai 平台層、
  不受影響（Tim 的帳號已用 API 金鑰方式接好 HackMD 連接器）。本機環境兩種都通，連接器
  不在就直接抓網頁。
...
- **「原筆記寫什麼」只能出現在 `data-note` 裡**，可見文字一律只講現在正確的敘述。
  Tim 2026-09-24 的原話：「所謂筆記應該是讓使用者學習的地方，甚至要說是教材，這種什麼『原筆記…』是你自己思考的對吧？應該要改掉」。
  他是讀者，不是來看我們怎麼查證的；查證軌跡放提示框，正文放教材。
- **「查證後確認原筆記是對的」這種反向註記不要寫進頁面。** 有教學價值的部分（限制、前提、為什麼）就直接當教材寫，
  沒有教學價值的就不寫。Tim 2026-09-24 拍板：正文只留教材。
...
- **課文是教材，不是工作紀錄。** 讀者看得見的每一個字都要在教東西。這幾種寫法一律不准出現在正文：
  「原筆記寫…」「查證後…」「這裡照它的用意處理」「這裡只留做法、不留產品名，因為…」，以及任何第一人稱
  （「我查到」「我們這裡」）。編輯過程中的取捨不必向讀者交代，要交代就寫進 `data-note` 或回報給 Tim。
  自我檢查：把 `data-note="…"` 整段拿掉之後，課文裡不該剩下「原筆記」三個字。
